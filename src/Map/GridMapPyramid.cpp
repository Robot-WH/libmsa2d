...
#include "msa2d/Map/GridMapPyramid.h"

namespace msa2d {
namespace map {

GridMapPyramid::GridMapPyramid(const Option& option) : option_(option) {
    ...
    for (unsigned int i = 0; i < option.num_depth; ++i) {
        ...
        /** 创建占用栅格地图 **/
        OccGridMap* occ_grid_map = new OccGridMap(
            mapResolution, map_grid_size, Eigen::Vector2f(map_in_world_.x(), map_in_world_.y()));
        mapOperateContainer_.push_back(OccGridMapOperation(occ_grid_map));
...
float GridMapPyramid::getScaleToMap() const { 
    return mapOperateContainer_[0].getScaleToMap(); 
} // 获取地图尺度 scale = 1.0 / map_resolution.

int GridMapPyramid::getMapLevels() const { 
    return mapOperateContainer_.size(); 
}

const OccGridMap& GridMapPyramid::getGridMap(int mapLevel) const { 
    return mapOperateContainer_[mapLevel].getGridMap(); 
};

std::mutex* GridMapPyramid::getMapMutex(int i) {
    return mapOperateContainer_[i].getMapMutex(); /// 获取指定层的地图锁
}

void GridMapPyramid::onMapUpdated() {
...
void GridMapPyramid::updateByScan(const std::vector<sensor::LaserPointContainer>& data_containers, 
...
bool GridMapPyramid::isCloseToBoundary(const Eigen::Vector3f& laser_pose_in_world) {
...
void GridMapPyramid::moveTo(const Eigen::Vector2f& new_map_pos_in_world) {
    for (uint16_t i = 0; i < mapOperateContainer_.size(); ++i) {
        mapOperateContainer_[i].moveTo(new_map_pos_in_world);
    }
}

void GridMapPyramid::setUpdateFactorFree(float free_factor) {
    size_t size = mapOperateContainer_.size();

    for (unsigned int i = 0; i < size; ++i) {
        OccGridMap &map = mapOperateContainer_[i].getGridMap();
        // map.setUpdateFreeFactor(free_factor);
    }
}

void GridMapPyramid::setUpdateFactorOccupied(float occupied_factor) {
    size_t size = mapOperateContainer_.size();
